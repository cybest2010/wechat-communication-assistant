...
| 用户操作 | 系统动作 |
|---------|---------|
| 复制推荐项 | 记录该方向被采用；调用 `/api/sample/save` 存为最佳样本 |
| 复制备选项 | 同上，同时记录推荐项未被采用 |
| 点「↺ 重新生成」 | 使用相同消息 + 当前目标重新调用 AI |
| 点「🎯 调整目标」 | 展开内联目标选择器，重新选择后生成 |
| 点「🙅 这不像我」 | 调用 `/api/feedback`（notLikeMe: true）后立即重新生成 |
| 长期不复制某方向 | 降低该方向在此场景权重（待实现） |
